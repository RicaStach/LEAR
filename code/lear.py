...
class ExperimentRun:
    ...
    def __init__(self, config_filepath):
        ...
        if not distributional_vectors:
            ...
            print(
                "You can download vectors from (Levy and Goldberg, 2014) at: http://u.cs.biu.ac.il/~yogo/data/syntemb/bow2.words.bz2")
        ...
        synonym_list_mono = self.config.get("data", "synonyms_asym").replace("[", "").replace("]", "").replace(" ",
                                                                                                               "").split(
            ",")
        synonym_list_cl = self.config.get("data", "synonyms_sym").replace("[", "").replace("]", "").replace(" ",
                                                                                                            "").split(
            ",")
    ...
    def initialise_model(self, numpy_embedding):
        ...
        negative_examples_attract_left_mono = tf.nn.l2_normalize(
            tf.nn.embedding_lookup(self.W_dynamic, self.negative_examples_attract_mono[:, 0]), 1)
        negative_examples_attract_right_mono = tf.nn.l2_normalize(
            tf.nn.embedding_lookup(self.W_dynamic, self.negative_examples_attract_mono[:, 1]), 1)
        ...
        attract_similarity_between_examples_mono = tf.reduce_sum(
            tf.multiply(attract_examples_left_mono, attract_examples_right_mono), 1)
        ...
        attract_similarity_to_negatives_left_mono = tf.reduce_sum(
            tf.multiply(attract_examples_left_mono, negative_examples_attract_left_mono), 1)
        ...
        attract_similarity_to_negatives_right_mono = tf.reduce_sum(
            tf.multiply(attract_examples_right_mono, negative_examples_attract_right_mono), 1)

        # and the final Attract Cost Function (sans regularisation):
        self.attract_cost_mono = tf.nn.relu(
            self.attract_margin_mono + attract_similarity_to_negatives_left_mono - attract_similarity_between_examples_mono) + \
                                 tf.nn.relu(
                                     self.attract_margin_mono + attract_similarity_to_negatives_right_mono - attract_similarity_between_examples_mono)
        ...
        cosine_similarity = tf.reduce_sum(
            tf.multiply(attract_examples_left_mono_normalised, attract_examples_right_mono_normalised), 1)
        ...
        # and their respective negative examples:
        negative_examples_attract_left_cl = tf.nn.l2_normalize(
            tf.nn.embedding_lookup(self.W_dynamic, self.negative_examples_attract_cl[:, 0]), 1)
        negative_examples_attract_right_cl = tf.nn.l2_normalize(
            tf.nn.embedding_lookup(self.W_dynamic, self.negative_examples_attract_cl[:, 1]), 1)

        # dot product between the example pairs. 
        attract_similarity_between_examples_cl = tf.reduce_sum(
            tf.multiply(attract_examples_left_cl_normalise, attract_examples_right_cl_normalise), 1)

        # dot product of each word in the example with its negative example. 
        attract_similarity_to_negatives_left_cl = tf.reduce_sum(
            tf.multiply(attract_examples_left_cl_normalise, negative_examples_attract_left_cl), 1)
        attract_similarity_to_negatives_right_cl = tf.reduce_sum(
            tf.multiply(attract_examples_right_cl_normalise, negative_examples_attract_right_cl), 1)

        # and the final Attract Cost Function (sans regularisation):
        self.attract_cost_cl = tf.nn.relu(
            self.attract_margin_cl + attract_similarity_to_negatives_left_cl - attract_similarity_between_examples_cl) + \
                               tf.nn.relu(
                                   self.attract_margin_cl + attract_similarity_to_negatives_right_cl - attract_similarity_between_examples_cl)
        ...
        regularisation_cost_attract_mono = self.regularisation_constant * (
                tf.nn.l2_loss(original_attract_examples_left_mono - attract_examples_left_mono) + tf.nn.l2_loss(
            original_attract_examples_right_mono - attract_examples_right_mono))
        self.attract_cost_mono += regularisation_cost_attract_mono
        self.attract_cost_mono = self.attract_cost_mono * 1.0

        regularisation_cost_attract_cl = self.regularisation_constant * (
                tf.nn.l2_loss(original_attract_examples_left_cl - attract_examples_left_cl) + tf.nn.l2_loss(
            original_attract_examples_right_cl - attract_examples_right_cl))
    ...
    def load_experiment_hyperparameters(self):
        ...
        print(
            ...
            self.attract_margin_value_mono, self.attract_margin_value_cl, self.repel_margin_value, self.batch_size,
            self.regularisation_constant_value, self.max_iter)
    ...
    def extract_negative_examples(self, list_minibatch, attract_batch=0):
        ...
        if attract_batch == 0:
            representations = self.sess.run([self.embedding_attract_left_cl, self.embedding_attract_right_cl],
                                            feed_dict={self.attract_examples_cl: list_minibatch})
        elif attract_batch == 1:
            representations = self.sess.run([self.embedding_attract_left_mono, self.embedding_attract_right_mono],
                                            feed_dict={self.attract_examples_mono: list_minibatch})
        else:
            representations = self.sess.run([self.embedding_repel_left, self.embedding_repel_right],
                                            feed_dict={self.repel_examples: list_minibatch})
    ...
    def attract_repel(self):
        ...
        print("\nAntonym pairs:", len(self.antonyms), "Synonym pairs (Symetric/CL):", len(self.synonyms_sym),
              "Synonym pairs (Asym/Mono)", len(self.synonyms_asym))
            ...
            for batch_index in range(0, batches_per_epoch):
                ...
                if syn_or_ant_batch == 0:
                    ...
                    self.sess.run([self.attract_cost_step_cl],
                                  feed_dict={self.attract_examples_cl: synonymy_examples_cl,
                                             self.negative_examples_attract_cl: current_negatives, \
                                             self.attract_margin_cl: self.attract_margin_value_cl,
                    ...
                    self.sess.run([self.attract_cost_step_mono],
                                  feed_dict={self.attract_examples_mono: synonymy_examples_mono,
                                             self.negative_examples_attract_mono: current_negatives, \
                                             self.attract_margin_mono: self.attract_margin_value_mono,
                                             self.regularisation_constant: self.regularisation_constant_value})
    ...
    def create_vector_dictionary(self):
        ...
        if self.log_scores_over_time or self.print_simlex:
            (score_simlex, score_wordsim) = simlex_scores(self.word_vectors, self.distance_metric, self.order,
                                                          self.print_simlex)
...
def hyperlex_analysis(word_vectors, language="english", source="hyperlex", distance_metric="metric-1", order=2,
                      add_prefixes=True):
    """
    This method computes the Spearman's rho correlation (with p-value) of the supplied word vectors.
    """
...
def simlex_scores(word_vectors, distance_metric, order, print_simlex=True):
    for language in ["english", "german", "italian", "russian", "croatian", "hebrew"]:
        ...
        if language == "english":
            ...
            hyperlex_score, hyperlex_coverage = hyperlex_analysis(word_vectors, language, source="hyperlex",
                                                                  distance_metric=distance_metric, order=order)
            hyperlex_nouns_score, hyperlex_nouns_coverage = hyperlex_analysis(word_vectors, language,
                                                                              source="hyperlex-nouns",
        ...
        if simlex_coverage > 0:
            ...
            if print_simlex:
                ...
                if language == "english":
                    ...
                    print("HyperLex score for", language, "is:", hyperlex_score, "coverage:", hyperlex_coverage,
                          "/ 2616")
                    print("HyperLex-Nouns score for", language, "is:", hyperlex_nouns_score, "coverage:",
                          hyperlex_nouns_coverage, "/ 2163")
...
def run_experiment(config_filepath):
    ...
    results directory.
    """
    # R. Stach: initialize the model
    current_experiment = ExperimentRun(config_filepath)

    # R. Stach: train the model
...
def main():
    ...
    try:
        ...
        config_filepath = r'..\config\experiment_parameters.cfg'
